...
- **Mascot mantle band** (`#b08846`) is slightly darker than the gold system color (`#C6A87C`). Minor, but worth aligning if a mascot polish pass happens.

## Suggested Next Step

- **Mascot hat refinement (Codex):** The hat proportions in the big SVG could be slightly more expressive — the brim line currently sits very close to the body/eye line. Worth considering a slight upward adjustment to give the hat more height and visual separation from the eyes.
- **Product card hover colour:** Currently all three cards share the same mint hover radial glow. Could give each card its own accent colour (mint for AniCal, coral for OctoQuiz, gold/warm for ArtisanMU) to add personality without breaking the system.
- **Mobile hero:** The brief flags that the mascot can block CTA on mobile. At `top: 430px` on mobile, it floats over the hero-actions. Could push to `top: 520px` or make it spawn below the fold on small screens.
